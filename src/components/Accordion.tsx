...
const Accordion: React.FC<AccordionProps> = ({ title, content, isActive, onToggle }) => {
  ...
  return (
    <div className="flex items-center flex-col sm:border-l border-[#707070] sm:pl-10">
      <button
        className={`flex items-center gap-4 sm:gap-8 w-full mb-2 ${isActive ? '' : ''}`}
        onClick={onToggle}
      >
        <Image
          src={isActive ? '/images/icons/dash.svg' : '/images/icons/plus.svg'}
          alt="icon"
          width={20}
          height={20}
        />
      ...
      <div
        ...
        className="pl-14 mb-3 overflow-hidden"
      >
